...
def build_import_plan(conn, *, batch_id: int) -> ImportPlan:
    plan = ImportPlan(batch_id=batch_id)
    batch = conn.execute(
    ...
    for enrollment_id, page_id, row_label, count, subject_id, count_status, subject_status in enrollment_rows:
        ...
        if subject_id is None or subject_status in ("PENDING", "AMBIGUOUS", "NOT_FOUND"):
            ...
            continue
        key = (camp_id, student_id, subject_id)
        ...
        existing = get_matching_camp_enrollments(conn, *key)
        ...
        if action == "INSERT":
            ...
            plan.conflicts.append(
                f"本登録に重複があります（camp={camp_id}, student={student_id}, subject={subject_id}, {len(existing)}件）"
            )
    ...
    staged_availability: dict[tuple[int, int], dict] = {}
    for availability_id, page_id, slot_id, value, status in availability_rows:
        student_id = page_students.get(page_id)
        if student_id is None:
            continue
        if status in ("PENDING", "AMBIGUOUS", "SLOT_NOT_FOUND", "OUT_OF_CAMP_RANGE") or slot_id is None or value is None:
        ...
        if len(existing) == 0:
            ...
            plan.conflicts.append(
                f"本登録の対応可能時間に重複があります（student={student_id}, slot={slot_id}, {len(existing)}件）"
            )
...
def execute_import_batch(
    ...
    *,
    ...
    plan = build_import_plan(conn, batch_id=batch_id)
    ...
    try:
        ...
        locked_plan = build_import_plan(conn, batch_id=batch_id)
        ...
        for item in locked_plan.enrollment_inserts:
            ...
            _mark_enrollment_imported(conn, item, target_id, batch_id, operator_instructor_id, operator_name, batch, now, None)
        for item in locked_plan.enrollment_updates:
            ...
            _mark_enrollment_imported(conn, item, target_id, batch_id, operator_instructor_id, operator_name, batch, now, existing)
        for item in locked_plan.availability_inserts:
            ...
            _mark_availability_imported(conn, item, cur.lastrowid, batch_id, operator_instructor_id, operator_name, batch, now, None)
        for item in locked_plan.availability_updates:
            existing = item["existing"]
            target_id = existing[0]
            conn.execute(
                "UPDATE CAMP_STUDENT_AVAILABILITY SET is_available=? WHERE availability_id=?",
                (item["is_available"], target_id),
            )
            _mark_availability_imported(conn, item, target_id, batch_id, operator_instructor_id, operator_name, batch, now, existing)
        conn.execute(
...
def _mark_enrollment_imported(conn, item, target_id, batch_id, operator_id, operator_name, batch, now, before):
    conn.execute(
    ...
    conn.execute(
        ...
        VALUES (:batch_id,:page_id,'IMPORT','CAMP_COURSE_ENROLLMENTS',:target_id,
        ...
        """,
        {
            ...
            "after_value_json": json.dumps({"contracted_count": item["contracted_count"]}, ensure_ascii=False),
...
def _mark_availability_imported(conn, item, target_id, batch_id, operator_id, operator_name, batch, now, before):
    ...
    conn.execute(
        ...
        INSERT INTO IMAGE_IMPORT_AUDIT_LOG
            (batch_id,page_id,action_type,target_table,target_id,before_value_json,after_value_json,
             actor_type,operator_instructor_id,operator_name,school_id,school_name,created_at)
        VALUES (:batch_id,:page_id,'IMPORT','CAMP_STUDENT_AVAILABILITY',:target_id,
                :before_value_json,:after_value_json,'INSTRUCTOR',:operator_id,:operator_name,
                :school_id,:school_name,:created_at)
        """,
        {
            ...
            "target_id": target_id,
            "before_value_json": json.dumps(before, ensure_ascii=False) if before else None,
            "after_value_json": json.dumps({"is_available": item["is_available"]}, ensure_ascii=False),
            "operator_id": operator_id,
            "operator_name": operator_name,
            "school_id": batch[0],
            "school_name": batch[1],
